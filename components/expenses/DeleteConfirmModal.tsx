...
export function DeleteConfirmModal({ expense, onClose, onConfirm }: DeleteConfirmModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-[#232629] border border-[#bec7d1] dark:border-[#3a3d40] rounded-2xl w-full max-w-[360px] shadow-xl p-6">
        ...
        <div className="bg-[#f3f4f5] dark:bg-[#1a1c1e] rounded-lg px-4 py-3 mb-5">
          <p className="text-[13px] font-medium text-[#191c1d] dark:text-[#e2e4e5]">
            {expense.note || expense.category}
          </p>
          <p className="font-mono text-[15px] font-semibold text-[#ba1a1a] mt-1 tabular-nums">
        ...
        <div className="flex gap-3">
          <button
            onClick={onClose}
            className="flex-1 py-2.5 border border-[#bec7d1] dark:border-[#3a3d40] text-[14px] font-medium text-[#3f4850] dark:text-[#9aacb5] rounded-lg hover:bg-[#f3f4f5] dark:hover:bg-[#2e3132] transition-colors"
          >
            Cancel
          </button>
          <button
